...
public protocol ThirdPartiesAlbumCellProtocol {
    func configure(item: AlbumViewModel)
...
open class AlbumCell: UICollectionViewCell, ThirdPartiesAlbumCellProtocol {
    
    public func configure(item: AlbumViewModel) {
        count = item.count ?? 0
        titleText = item.name
        //Configure ImageView Image
        if let thumbnailURL = item.coverURL {
